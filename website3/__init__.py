...
from .forms import UserForm,  PostForm, LoginForm, SearchForm , CrawlPostForm
...
def start_crawling(category_url, category_name):
    ...
    # 각 뉴스 기사의 <li> 태그들을 선택합니다.
    news_articles = soup.select('ul.list_news._infinite_list > li.bx')

    # 순차적으로 선택된 뉴스 기사에 대해 작업을 수행합니다.
    for article in news_articles:
        # 각 뉴스 기사에 대한 작업을 수행합니다.
        # 예를 들어, 기사 제목과 링크를 가져오는 등의 작업을 수행할 수 있습니다.
    ...
    for  ann in announcements:
        if ann['title']=='' or ann['url'] =='' or ann['content']=='' or category_name is None:
            ...
            oldcategory = Categories.query.filter_by(title=ann['title']).first()
            ...
            #고유하다면 데이터배이스에 추가 category title content url
                ...
                #same title이 존재하지만 내가 crawling한게 아닌 경우 이 떄는 넣어도됨
                #현재 유저와 데이터베이스에 저장된 카테코리의 유저의 아이디나 이름 이런게 동일하면 넣지말고 동일하지않으면 넣으면됨
                if categorier == oldcategory.categorier_id:
