...
from .forms import UserForm, PasswordForm, PostForm, LoginForm, SearchForm , CrawlPostForm
...
def add_crawlpost():
    ...
    if form.validate_on_submit():
        ...
        if OldCategory is None:
            ...
            post = Posts(title = OldCategory.title, content = OldCategory.content.data, poster_id = poster,slug = OldCategory.category, url = OldCategory.url)
            #양식 초기화
            form.title.data = ''
    ...
    return render_template('add_crawlpost.html',categories = categories)
...
def start_crawling(url, category_name):
    ...
    if title is None or content is None or url is None or category_name is None:
        flash('error')
    else:

        #title이 데이터 베이스에 동일한게 존재하는지 확인하고 없다면 넣는다
        oldcategory = Categories.query.filter_by(title=title).first()
        if oldcategory is None:
            #고유하다면 데이터배이스에 추가 category title content url
            categorier = current_user.id
            category = Categories(category = category_name, title = title, content = content , url = url ,categorier_id = categorier)
            db.session.add(category)
            db.session.commit()
        else:
            flash("Same title exists")

    #message = f"Your Category title is {title}! url은 {url}, 발행 시간은 {time}입니다."
    #flash(message)    
    ...
    if request.method == 'POST':
        ...
        if category_name == "Choose...":
            ...
            if category_name == "Music":
                url = "https://comic.naver.com/webtoon/weekday"
            if category_name == "Stock":
                url = "https://comic.naver.com/webtoon/weekday"
            start_crawling(url, category_name)
